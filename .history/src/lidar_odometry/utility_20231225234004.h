...
typedef pcl::PointCloud<pcl::PointXYZINormal>::ConstPtr PointCloudConstPtr;
...
void ExtractNormals(const PointCloudConstPtr& input,
  ...
  if (input->points[0].normal_x == 0 && input->points[0].normal_y == 0 &&
    ...
    return ComputeNormals<PointType>(input, params, normals);
  }
...
void ComputeKeypoints(const PointCloudConstPtr& source,
  ...
  pcl::HarrisKeypoint3D<PointType, Point> harris_detector;
  ...
  harris_detector.setMethod(
      static_cast<pcl::HarrisKeypoint3D<PointType, PointType>::ResponseMethod>(
...
void ComputeFeatures(const PointCloudConstPtr& keypoints,
  ...
  pcl::search::KdTree<PointType>::Ptr search_method(new pcl::search::KdTree<Point>);
