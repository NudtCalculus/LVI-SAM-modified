...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    // std::cout << "A scancontext has been saved." << std::endl;
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0) // to save computation cost
    {
        ...
        polarcontext_tree_ = boost::make_unique<InvKeyTree>(PC_NUM_RING /* dim */, polarcontext_invkeys_to_search_, 10 /* max leaf */ );        // tree_ptr_->index->buildIndex(); // inernally called in the constructor of InvKeyTree (for detail, refer the nanoflann and KDtreeVectorOfVectorsAdaptor)
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        // std::cout << "polarcontext_candidate : \n" << polarcontext_candidate << std::endl; 
